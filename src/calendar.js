    ...
    .controller('uiCalendarCtrl', ['$scope', '$locale',
        function ($scope, $locale) {
            var sources = $scope.eventSources,
                ...
                eventSerialId = 1, sourceSerialId = 1, sourceEventsSerialId = 1,
            ...
            this.eventFingerprint = function (e) {
                var extraSignature = extraEventSignature({event: e}) || '',
                    start = moment.isMoment(e.start) ? e.start.unix() : (e.start ? moment(e.start).unix() : ''),
                    end = moment.isMoment(e.end) ? e.end.unix() : (e.end ? moment(e.end).unix() : '');
                if (!e._id) {
                    e._id = eventSerialId++;
                }
            ...
            this.allEvents = function () {
                ...
                var source, extEvent, arraySources = [];
            ...
            this.changeWatcher = function (arraySource, tokenFn) {
                var self,
                    ...
                    applyChanges = function (newTokens, oldTokens) {
                        var i, n, el, token, removedToken, newToken, addedTokens,
                            replacedTokens = {},
                            removedTokens = subtractAsSets(oldTokens, newTokens);
                        for (i = 0, n = removedTokens.length; i < n; i++) {
                            removedToken = removedTokens[i];
                            el = map[removedToken];
                        ...
                        addedTokens = subtractAsSets(newTokens, oldTokens);
                ...
                link: function (scope, elm, attrs, controller) {
                    ...
                    function getOptions () {
                        var calendarSettings = attrs.uiCalendar ? scope.$parent.$eval(attrs.uiCalendar) : {},
                            fullCalendarConfig, localeFullCalendarConfig, options2 = {};
                        fullCalendarConfig = controller.getFullCalendarConfig(calendarSettings, uiCalendarConfig);

                        localeFullCalendarConfig = controller.getLocaleConfig(fullCalendarConfig);
                    ...
                    eventsWatcher.onChanged = function (event) {
                        ...
                        if (calendar && calendar.fullCalendar) {
                            clientEvents = calendar.fullCalendar('clientEvents', event._id);
                            for (var i = 0; i < clientEvents.length; i++) {
                                clientEvent = clientEvents[i];
